...
parser.add_argument('--convert_L', action='store_true',
                    help='convert to L?')
...
def main():
    ...
    if not opt.test:
        train_set = get_training_set(opt.dataset, opt.add_noise, opt.convert_L,opt.crop)
...
def adjust_learning_rate(epoch):
    """Sets the learning rate to the initial LR decayed by 2 every step epochs"""
...
def validate(model, criterion, validating_data_loader,PSNR,RMSE):
    ...
    for batch in validating_data_loader:
        ...
        input0 = torch.zeros(input.shape[0],2,input.shape[2],input.shape[3])
        input0[:, 0, :, :] = input[:, 0, :, :]
        input0[:, 1, :, :] = input[:, 0, :, :]
        input1 = torch.zeros(input.shape[0], 2, input.shape[2], input.shape[3])
        input1[:, 0, :, :] = input[:, 1, :, :]
        input1[:, 1, :, :] = input[:, 1, :, :]
        input2 = torch.zeros(input.shape[0], 2, input.shape[2], input.shape[3])
        input2[:, 0, :, :] = input[:, 2, :, :]
        input2[:, 1, :, :] = input[:, 2, :, :]
        model_out0 = model(input0)
...
def test(model, criterion, testing_data_loader,PSNR,RMSE):
    ...
    for batch in testing_data_loader:
        ...
        input0[:, 0, :, :] = input[:, 0, :, :]
        input0[:, 1, :, :] = input[:, 0, :, :]
        #print('input0_shape:', input0.shape)
        input1 = torch.zeros(input.shape[0], 2, input.shape[2], input.shape[3])
        input1[:, 0, :, :] = input[:, 1, :, :]
        input1[:, 1, :, :] = input[:, 1, :, :]
        input2 = torch.zeros(input.shape[0], 2, input.shape[2], input.shape[3])
        ...
        input2[:, 1, :, :] = input[:, 2, :, :]
        model_out0 = model(input0)
        #print('model_out0_shape:', model_out0.shape)
        model_out1 = model(input1)
        model_out2 = model(input2)
        #prediction = torch.zeros(target[:,0,:,:].shape).cuda()
        prediction = torch.zeros(target.shape).cuda()
        ...
        prediction[:, 2, :, :] = torch.addcmul(prediction[:, 2, :, :], 1, model_out2[:, 0, :, :], input[:, 2, :, :]) \
                                 + model_out2[:, 1, :, :]
        #mse = criterion(prediction, target[:,0,:,:])
        mse = criterion(prediction, target)
        rmse = sqrt(mse.item())
        psnr = 10 * log10(1.0 / mse.item())
        avg_rmse += rmse
...
def test(model, criterion, testing_data_loader):
    ...
    for batch in testing_data_loader:
        ...
        if opt.cuda:
            input = input.cuda()
            target = target.cuda()
        save_image(target, num, 'denoising_inputs')
        input0 = torch.zeros(target.shape[0], 2, target.shape[2], target.shape[3])
        input0[:, 0, :, :] = target[:, 0, :, :]
        input0[:, 1, :, :] = target[:, 0, :, :]
        input1 = torch.zeros(target.shape[0], 2, target.shape[2], target.shape[3])
        input1[:, 0, :, :] = target[:, 1, :, :]
        input1[:, 1, :, :] = target[:, 1, :, :]
        input2 = torch.zeros(target.shape[0], 2, target.shape[2], target.shape[3])
        input2[:, 0, :, :] = target[:, 2, :, :]
        input2[:, 1, :, :] = target[:, 2, :, :]
        model_out0 = model(input0)
        model_out1 = model(input1)
